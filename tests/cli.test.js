...
jest.mock("@lzwme/fe-utils", () => {
  return {
    color: require("console-log-colors"),
    Request: {
      getInstance: () => ({
        get: jest.fn((url) => {
          return new Promise((resolve) => {
            setTimeout(
              () => resolve({ response: { statusCode: url.includes("error.com") ? -1 : 200 } }),
              (Math.random() + 1) * 1000
            );
          });
        }),
...
describe("nrm command which needs to add a custom registry", () => {
  ...
  it("nrm set <name>", async () => {
    ...
    await coffee
      .spawn("nrm", [`set ${__REGISTRY__} -a ${attr} -v ${value}`], {
        shell: isWin,
      })
  ...
  it("login <name> [base64]", async () => {
    ...
    await coffee
      .spawn("nrm", [`login ${__REGISTRY__} -u ${username} -p ${password}`], {
        shell: isWin,
      })
      .expect("stdout", /success/g)
      .expect("code", 0)
